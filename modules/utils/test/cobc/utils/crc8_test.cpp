 ...
 */
TEST(Crc8Test, rmapTestPattern1)
{
    uint8_t data[] = {
        0x01, 0x02, 0x03, 0x04,
        0x05, 0x06, 0x07, 0x08
    ...
    EXPECT_EQ(0xB0U, Crc8::calculate(cobc::toArray(data)));
}
 ...
 * ECSS-E-50-11 Draft F (Version from December 2006)
 */
TEST(Crc8Test, rmapTestPattern2)
{
    uint8_t data[] = {
        0x53, 0x70, 0x61, 0x63,
        0x65, 0x57, 0x69, 0x72,
    ...
    EXPECT_EQ(0x84U, Crc8::calculate(cobc::toArray(data)));
}
 ...
 * ECSS-E-50-11 Draft F (Version from December 2006)
 */
TEST(Crc8Test, rmapTestPattern3)
{
    uint8_t data[] = {
        0x10, 0x56, 0xC3, 0x95,
        0xA5, 0x75, 0x38, 0x63,
    ...
    EXPECT_EQ(0x18U, Crc8::calculate(cobc::toArray(data)));
}

/**
 * Test the result of a CRC over the message + CRC, should be 0
 */
TEST(Crc8Test, rmapTestPattern3Total)
{
    uint8_t data[] = {
        0x10, 0x56, 0xC3, 0x95,
        0xA5, 0x75, 0x38, 0x63,
    ...
    EXPECT_EQ(0U, Crc8::calculate(cobc::toArray(data)));
}

/**
 * see ECSS‐E‐ST‐50‐52C Appendix A.4 (February 2010)
 */
TEST(Crc8Test, rmapCrcTestPattern1)
{
    uint8_t header[] = {
        0xFE, 0x01, 0x6C, 0x00,
        0x67,
    ...
    EXPECT_EQ(0x9FU, Crc8::calculate(cobc::toArray(header)));
    EXPECT_EQ(0x56U, Crc8::calculate(cobc::toArray(data)));
}
 ...
 */
TEST(Crc8Test, rmapCrcTestPatternCommand)
{
    uint8_t header[] = {
        0xFE, 0x01, 0x4D, 0x00,
    ...
    EXPECT_EQ(0xF7U, Crc8::calculate(cobc::toArray(header)));
}
 ...
 */
TEST(Crc8Test, rmapCrcTestPatternReply)
{
    uint8_t header[] = {
        0x67, 0x01, 0x0D, 0x00,
        0xFE,
