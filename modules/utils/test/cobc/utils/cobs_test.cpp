...
{
public:
    ConsistentOverheadByteStuffing cobs;
};

TEST_F(CobsTest, initialBehavoir)
{
    EXPECT_TRUE(cobs.isFinished());
}
...
TEST_F(CobsTest, emptyStringIsEncodedAsEmptyString)
{
    ...
    EXPECT_EQ(0x01, cobs.getNextByte());
    EXPECT_TRUE(cobs.isFinished());
}
...
TEST_F(CobsTest, encodingOfSingleBlockWithoutZero)
{
    ...
    cobs.start(input, sizeof(input));
    size_t encodedLength = cobs.getEncodedArray(actual, sizeof(actual));
