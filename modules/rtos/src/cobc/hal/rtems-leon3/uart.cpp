...
cobc::leon3::UART::UART(uint8_t id, uint32_t baudrate, uint8_t blocking) :
        devHandle(0), devId(id), baud(baudrate), devState(CLOSE), txCount(0),
        rxCount(0), blockingMode(blocking)
{
}
...
{
    devHandle = ::open(uartPath[this->devId], O_RDWR);
    if (devHandle < 0)
    {
        DEBUG("Device not opened, %s:errno-> %d\n",
                uartPath[this->devId], errno);
        return;
    }

    // Configure Device
    if (::ioctl(devHandle, APBUART_SET_BAUDRATE, baud) < 0)
        return;
    if (::ioctl(devHandle, APBUART_SET_ASCII_MODE, 0) < 0)
        return;
    if (::ioctl(devHandle, APBUART_CLR_STATS, 0) < 0)
        return;
    if (::ioctl(devHandle, APBUART_SET_RXFIFO_LEN, 64) < 0)
        return;
    if (::ioctl(devHandle, APBUART_SET_TXFIFO_LEN, 64) < 0)
        return;
    if (::ioctl(devHandle, APBUART_SET_BLOCKING, blockingMode) < 0)
        return;

    this->devState = OPEN;
    this->dev_enable();
    DEBUG("%s opened, %d\n", uartPath[this->devId], devState);
}

void
cobc::leon3::UART::close()
{
    int retval = 0;
    this->dev_disable();
...
std::size_t
cobc::leon3::UART::read(uint8_t* data,
                        std::size_t length,
...
std::size_t
cobc::leon3::UART::write(const uint8_t* data,
                         std::size_t length,
                         time::Duration timeout)
{
    (void) timeout;
